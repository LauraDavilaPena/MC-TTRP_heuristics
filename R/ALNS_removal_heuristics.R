...
random_removal <- function(n, phi, current_solution){
  
  # removed_candidates <- numeric()
  # for(i in 1:n){
  #   if(length(route_of_client(i,current_solution)$route)>3){
...
# Modificacion hecha 04/08/2021
worst_removal <-  function(input, current_solution, phi){
  clients <- input$n-1
  ...
  for(i in 1:clients){
    if(sum(i==rutas)>1){
      ...
      i_dist <- input$matriz.distancia[rutas[i_pos[1]-1]+1, rutas[i_pos[1]]+1] + input$matriz.distancia[rutas[i_pos[i_times]]+1, rutas[i_pos[i_times]+1]+1]
...
# Nuevo 04/08/2021
worst_extended_removal <-  function(input, current_solution, phi){
  clients <- input$n-1
  rutas <- all_routes(current_solution)
  dist <- numeric(clients)
  ...
  for(i in 1:clients){
    if(sum(i==rutas)>1){
      ...
      for(j in 1:i_times){
        i_dist[j] <- input$matriz.distancia[rutas[i_pos[j]-1]+1, rutas[i_pos[j]]+1] + input$matriz.distancia[rutas[i_pos[j]]+1, rutas[i_pos[j]+1]+1]
      }
      i_dist[i_times+1] <- input$matriz.distancia[rutas[i_pos[1]-1]+1, rutas[i_pos[1]]+1] + input$matriz.distancia[rutas[i_pos[i_times]]+1, rutas[i_pos[i_times]+1]+1]
...
shaw_removal <- function(input, n, phi){
  ...
  removed_clients <- c(chosen_client,removed_clients)
...
pb_removal <- function(input, n, phi){
...
neighborhood_removal <- function(input, phi, current_solution){
...
shaw.ali_removal <- function(input, n, phi, current_solution){
