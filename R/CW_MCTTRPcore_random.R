...
CW_MCTTRPcore_random<-function(matriz.demandas, matriz.distancia, capacidad.truck, capacidad.trailer,
  ...
  while(Sm>0){
    ...
    Sm<-sample(c(sample(Shat[Shat > 0],1),sample(S[S>0],1)),1) #Escogemos el valor maximo de la matriz de ahorros
    merge <- 0
